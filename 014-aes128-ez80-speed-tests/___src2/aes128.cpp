...
char msg[1024], key[32], outmsg[1024];
...
byte *aes128_crypt(byte* msgInputArr, byte* keyArr, byte *msgOutputArr, int msgLen)
{
	...
	for(i = 0; i < ml; i += 16)
	{
		aes128_calculate(msgInputArr + i, keyArr, msgOutputArr + i);
		//printf("1) ");
	...
	{
		//printf("[last block]\n");
		for(int j = msgLen, i = 16 - (msgLen & 0x0f); i < 16; i++, j++)
			msgInputArr[j] = '\0';
		
		aes128_calculate(msgInputArr + ml, keyArr, msgOutputArr + ml);
		//printf("2) ");
...
void aes_key_expansion(byte *keyArr, ulong *keyScheduleArr)
{
	...
	for(byte i = Nk; i <= 4 * Nr + 3; i++)
	{
		...
		if(i % Nk == 0)
		{
			...
			temp = aes_sub_word(aes_rot_word(temp)) ^ (Rcon[i / Nk] << 24);
		}
		else if(Nk > 6 && (i % Nk == 4))
