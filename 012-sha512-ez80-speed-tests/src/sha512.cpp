/****************************************************************
	2024/09/09 13:11 NZT
	SHA-512 software implementation

	(following implementation from the book "" and my previous
	  implementation adapted to Microsoft C#)
	* Translated into Agon Light (eZ80F92) SDK and data types.
...
char* sha512_hash(byte *arrData, long totalDataLen)
{
	lastBlock = false;
	...
	// Initialise internal state for SHA-512 calculations (different from SHA-224, SHA-256 and SHA-384 initial constants).
    ...
    // Pack 8 contiguous bytes (8 8-bits, in LSB format) into one M array cell (with 64-bits, in LSB format)
	...
	strcat(finalHashArr, NumTools_toHex16(h[6]));     // Used only on SHA-512, not on SHA-384
	strcat(finalHashArr, NumTools_toHex16(h[7]));     // Used only on SHA-512, not on SHA-384

	return finalHashArr;
}


void sha512_calculate(ulong64 *W)
{
